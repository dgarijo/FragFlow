...
int main (int argc, char **argv)
{
   ...
   FILE *outputFile, *outputFileOccurrences;
   ULONG iteration;
   BOOLEAN done;
      ...
      while ((iteration <= parameters->iterations) && (!done))
      {
         ...
         else
         {
            ...
            if (parameters->outputToFile)
            {
               ...
               //code added by Dani: auxiliary file for printing the occurrences
               char * s = malloc(snprintf(NULL, 0, "%s_occurrences",  parameters->outFileName) + 1);
               sprintf(s, "%s_occurrences", parameters->outFileName);
               outputFileOccurrences = fopen(s, "a");
               //end of code added by Dani
               if (outputFile == NULL)
               {
               ...
               //Code edited by Daniel
               //Only write if the number of instances of the best substructure is >1               
               if(subList->head->sub->numInstances>1){
                   WriteGraphToFile(outputFile, subList->head->sub->definition,
                                    parameters->labelList, 0, 0,
                                    subList->head->sub->definition->numVertices,
                                    TRUE);
                   fclose(outputFile);
                   //Code edited by Daniel: for each best substructure, we annotate
                   //the number of occurrences. This number is stored in another file
                   fprintf(outputFileOccurrences, "SUB_%lu -> %lu\n",iteration, subList->head->sub->numInstances);
                   fclose(outputFileOccurrences);
               }
            }            
            ...
            //Code edited by Daniel: if we find only an instance of the best substructure, we are done.
            if(subList->head->sub->numInstances<=1){
